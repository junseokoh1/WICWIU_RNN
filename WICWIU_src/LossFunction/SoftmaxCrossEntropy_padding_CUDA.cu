...
#include "SoftmaxCrossEntropy_padding.hpp"

// template class SoftmaxCrossEntropy_padding<int>;
template class SoftmaxCrossEntropy_padding<float>;
// template class SoftmaxCrossEntropy_padding<double>;
...
template<typename DTYPE> Tensor<DTYPE> *SoftmaxCrossEntropy_padding<DTYPE>::ForwardPropagateOnGPU(int pTime) {
...
__global__ void SoftmaxCrossEntropy_BackPropagate_kernel(int time, int batchIndex, int capacity, float *input_delta, float *label, float *softmaxresult) {
    int idx = 0;
...
template<typename DTYPE> Tensor<DTYPE> *SoftmaxCrossEntropy_padding<DTYPE>::BackPropagateOnGPU(int pTime) {
    Tensor<DTYPE> *label         = this->GetLabel()->GetResult();
    Tensor<DTYPE> *softmaxresult = m_aSoftmaxResult;
    ...
    // std::cout<<"SoftmaxCrossEntropy_padding<DTYPE>::BackPropagateOnGPU"<<'\n';
    ...
    for(int ba = 0; ba < batchsize; ba++){
        ...
        SoftmaxCrossEntropy_BackPropagate_kernel << < noBlock, threadsPerBlock >> > (0, ba, capacity, pDevInputDelta, pDevLabel, pDevSoftMax);
