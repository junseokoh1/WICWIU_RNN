#ifndef HIDDENMATMUL_H_
#define HIDDENMATMUL_H_    value

#include "../Operator.hpp"
#include <cstdio>

template<typename DTYPE> class HiddenMatMul : public Operator<DTYPE>{
private:
#ifdef __CUDNN__
...
public:
    ...
    */
    HiddenMatMul(Operator<DTYPE> *pWeight, Operator<DTYPE> *pInput, std::string pName, int pLoadflag = TRUE) : Operator<DTYPE>(pWeight, pName, pLoadflag) {
        #ifdef __DEBUG__
        std::cout << "HiddenMatMul::HiddenMatMul(Operator<DTYPE> *, Operator<DTYPE> *, std::string)" << '\n';
        #endif  // __DEBUG__
    ...
    @ref void Delete()
    */
    virtual ~HiddenMatMul() {
        #ifdef __DEBUG__
        std::cout << "HiddenMatMul::~HiddenMatMul()" << '\n';
        #endif  // __DEBUG__
        Delete();
    }
    ...
    */
    int Alloc(Operator<DTYPE> *pWeight, Operator<DTYPE> *pInput) {
        #ifdef __DEBUG__
        std::cout << "HiddenMatMul::Alloc(Operator<DTYPE> *, Operator<DTYPE> *)" << '\n';
        #endif  // __DEBUG__
...
*/
                        ...
                        for (int hid = 0; hid < hiddensize; hid++) {
                            (*result)[Index5D(resultTenShape, ti, ba, ch, ro, co)]
                                += (*weight)[Index5D(weightTenShape, 0, 0, 0, co, hid)]
                                   * (*input)[Index5D(inputTenShape, ti-1, ba, ch, ro, hid)];
                ...
                for (int ro = 0; ro < rowsize; ro++) {
                    for (int co = 0; co < colsize; co++) {
                        for (int hid = 0; hid < hiddensize; hid++) {
                            ...
                            input_index  = Index5D(inputTenShape, ti+1, ba, ch, ro, hid);
    ...
    */
    int ForwardPropagateOnGPU(int pTime = 0) {
        ...
        m_pDevInput  = input->GetGPUData(pTime-1);
