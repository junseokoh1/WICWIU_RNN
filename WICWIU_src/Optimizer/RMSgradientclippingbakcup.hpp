    ...
    int UpdateParameter(Operator<DTYPE> *pParameter, Tensor<DTYPE> *m_pMeanSquared) {
      ...
      //  }
        ...
        for (int i = 0; i < capacity; i++) {
            ...
            //gradient clipping을 여기에 추가해보자
            //check = (*gradient)[i]) / std::sqrt((*m_pMeanSquared)[i] + m_epsilon);

            if(abs( ( (*gradient)[i] / std::sqrt((*m_pMeanSquared)[i] + m_epsilon)) )  > m_clipValue){
                //(*trainable_data)[i] += (signed_learning_rate *(m_clipValue/abs((*gradient)[i] / std::sqrt((*m_pMeanSquared)[i] + m_epsilon) ))) * (*gradient)[i] / std::sqrt((*m_pMeanSquared)[i] + m_epsilon);
                (*trainable_data)[i] += (signed_learning_rate *(m_clipValue/abs((*gradient)[i] / std::sqrt((*m_pMeanSquared)[i] + m_epsilon) ))) * ((*gradient)[i] / std::sqrt((*m_pMeanSquared)[i] + m_epsilon));
            }
            else{
